...
export const issueColumns = pgTable(
  "issue_columns",
  {
    id: text("id").notNull().default(sql`gen_random_uuid()`).primaryKey(),
  ...
  (table) => [uniqueIndex("issue_columns_project_name_idx").on(table.projectId, table.name)]
);

export const issues = pgTable("issues", {
  id: text("id").notNull().default(sql`gen_random_uuid()`).primaryKey(),
  projectId: text("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  columnId: text("column_id").notNull().references(() => issueColumns.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  description: text("description").notNull().default(""),
